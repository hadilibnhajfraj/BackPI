...
const lesmatiere = require("../model/lesmatiere");
...
const lesclass = require("../model/lesclass");
  ...
  try {
    ...
    if (existingEmploi) {
      return res.status(400).json({
        message:
          "Un emploi existe déjà pour cette classe et cette date de début.",
      });
    }
    ...
    res
      .status(200)
      .json({ _id: emploi._id, message: "Emploi ajouté avec succès." });
  } catch (err) {
    console.log(err);
    res
      .status(500)
      .send("Une erreur s'est produite lors de l'ajout de l'emploi.");
  }
}
  ...
  try {
    upload(req, res, async function (err) {
      if (err instanceof multer.MulterError) {
        return res
          .status(500)
          .json({ message: "An error occurred while11111 uploading files." });
      } else if (err) {
        console.log(err);
        return res
          .status(500)
...
async function deleted(req, res, next) {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while deleting the emploi" });
  }
}
...
async function addSeance(req, res, next) {
  try {
    ...
    if (seanceDate < emploiDateDebut || seanceDate > emploiDateFin) {
      return res
        .status(400)
        .json({ message: "Seance date is out of the employment period" });
    }
    ...
    const matiereDoc = await lesmatiere.findById(matiere);
    if (!matiereDoc) {
    ...
    res
      .status(200)
      .json({ message: "Seance added successfully", seance: newSeance });
  } catch (err) {
    console.error(err);
    res
      .status(500)
      .json({ message: "An error occurred while adding the seance" });
  }
}
...
async function extendEmploie(req, res, next) {
  try {
    ...
    if (additionalWeeks > 10) {
      return res
        .status(400)
        .send("Le nombre de semaines supplémentaires ne peut pas dépasser 10.");
    }
    ...
    res
      .status(500)
      .send("Une erreur est survenue lors de l'extension de l'emploi.");
  }
}
...
const generateTimetablePDF = async (emploiId) => {
  try {
    ...
    emploi.seances.forEach((seance) => {
      ...
      const startTime = `${seance.heure_debut
        .toString()
        .padStart(2, "0")}:00 - ${seance.heure_fin
        .toString()
        .padStart(2, "0")}:00`;
      ...
      if (timetable[seanceDay] && timetable[seanceDay][startTime]) {
        ...
        console.log(
          `Combinaison invalide de jour et d'heure: ${seanceDay}, ${startTime}`
        );
      }
    });
    ...
    doc.text(
      `Emploi du temps de la classe ${
        emploi.class.name
      } du ${emploi.date_debut.toDateString()} au ${emploi.date_fin.toDateString()}`,
      { align: "center" }
    );
    ...
    dates.forEach((date, rowIndex) => {
      doc.text(date, tableX - 40, tableY + (rowIndex + 1) * cellHeight, {
        width: 40,
        align: "center",
      }); // Ajuster la position des jours
    });

    // Ajouter les heures au-dessus des séances
    seanceTimes.forEach((time, index) => {
      doc.text(time, tableX + index * cellWidth, tableY, {
        width: cellWidth,
        align: "center",
      }); // Ajuster la position des heures
    });
    ...
    dates.forEach((date, rowIndex) => {
      seanceTimes.forEach((time, colIndex) => {
        ...
        seances.forEach((seance) => {
          rowData += `${seance.lesmatiere}\n${seance.enseignant}\n${seance.salle}\n\n`;
        });
        // Dessiner une bordure autour de chaque cellule
        doc
          .rect(
            tableX + colIndex * cellWidth,
            tableY + (rowIndex + 1) * cellHeight,
            cellWidth,
            cellHeight
          )
          .stroke();
        ...
        doc
          .fontSize(seanceFontSize)
          .text(
            rowData,
            tableX + colIndex * cellWidth + 2,
            tableY + (rowIndex + 1) * cellHeight + 2,
            { width: cellWidth - 4, height: cellHeight - 4, align: "center" }
          );
      });
    });
...
const generateTimetableForTeacherPDF = async (emploiId, enseignantId) => {
  try {
    ...
    const filteredSeances = allSeances.filter(
      (seance) => seance.enseignant._id.toString() === enseignantId
    );
    ...
    let emploiEnseignant = await EmploiEnseignant.findOne({
    ...
    while (currentDate <= new Date(emploi.date_fin)) {
      ...
      dates.push(
        ...Object.keys(joursSemaine).map((date) =>
          joursSemaine[date].toDateString()
        )
      );
    ...
    const enseignant = await User.findById(enseignantId).select(
      "firstName lastName"
    );
    const enseignantName = enseignant
      ? `${enseignant.firstName}_${enseignant.lastName}`
      : "unknown";
    ...
    // Ajouter le titre centré
    doc.text(
      `Emploi du temps de l'enseignant ${enseignantName} du ${emploi.date_debut.toDateString()} au ${emploi.date_fin.toDateString()}`,
      { align: "center" }
    );
...
async function getEmailsOfClassStudents(emploiId) {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching the data." }); // ou une autre indication d'erreur que vous préférez
  }
}
...
async function getEmailsOfParentsOfClassStudents(emploiId) {
  try {
    ...
    const parentsEmails = await User.find(
      { _id: { $in: parentIds } },
      { email: 1, _id: 0 }
    );
...
async function getTeacherIdsByEmploiId(id) {
  try {
    ...
    throw new Error(
      "Une erreur est survenue lors de la récupération des IDs des enseignants : " +
        error.message
    );
  }
}
...
const generateAndSendGlobalTimetable = async (req, res) => {
  try {
    ...
    await Promise.all(
      parentEmails.map((email) =>
        sendEmail(
          ...
          "Voici l'emploi du temps de votre enfant.",
          globalTimetablePath
        )
      )
    );
    ...
    for (const teacherId of teacherIds) {
      const teacherPDFPath = await generateTimetableForTeacherPDF(
        emploiId,
        teacherId
      );
    ...
    res
      .status(200)
      .send({ message: "Emploi du temps généré et envoyé avec succès." });
  } catch (error) {
...
const emploiByParent = async (req, res) => {
  try {
    ...
    const etudiants = await Etudiant.find({ id_user: parentId }).populate(
      "classe"
    );
    ...
    const emplois = await Emploi.find({ class: { $in: classeIds } }).populate(
      "class seances"
    );
    ...
    console.error(
      "Erreur lors de la récupération des emplois par parent :",
      error
    );
...
async function addMatiere(req, res, next) {
  try {
    const matiere = new lesmatiere(req.body);
    await matiere.save();
...
async function showMatiere(req, res, next) {
  try {
    const data = await lesmatiere.find();
    res.status(200).json(data);
  } catch (err) {
    console.log(err);
  }
