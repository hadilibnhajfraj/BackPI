...
async function showEtudiant(req, res, next) {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Erreur lors de la récupération des données étudiantes.",
      });
  }
}
...
const changeStudentClass = async (req, res, next) => {
  ...
  try {
    ...
    console.log(
      `Found student: ${student._id}, current class: ${student.classe}`
    );
    ...
    if (currentClassId === newClassId) {
      console.log(
        "The new class is the same as the current class. No changes needed."
      );
      return res
        .status(400)
        .json({ error: "The new class is the same as the current class." });
    }
    ...
    res
      .status(500)
      .json({ error: "An error occurred while changing the student's class" });
  }
};
