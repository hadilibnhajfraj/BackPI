...
exports.login = async (req, res) => {
  try {
    ...
    try {
      ...
      const token = jwt.sign(
        { userId: user._id, role: user.authorities },
        jwtkey,
        { expiresIn: "1h" }
      );
      res.json({ token, role: user.authorities });
    } catch (e) {
...
exports.createUser = async (req, res) => {
  try {
    ...
    const newUser = new User({
      ...
      activated: false, // Désactivé par défaut
      validatedByAdmin: false, // Non validé par défaut
    });
    await newUser.save();
    res
      .status(201)
...
exports.validateUser = async (req, res) => {
  try {
    ...
    if (user.authorities.includes("admin")) {
      return res
        .status(400)
        .json({ message: "Admin users cannot be validated" });
    }

    // Vérifier si l'utilisateur est de type parent ou enseignant
    if (
      !user.authorities.includes("parent") &&
      !user.authorities.includes("enseignant")
    ) {
      return res
        .status(400)
        .json({ message: "Only parent or teacher users can be validated" });
    }

    // Mettre à jour les propriétés de l'utilisateur pour la validation
    user.validatedByAdmin = true;
    user.activated = true; // Cela dépend de la logique de votre application, si nécessaire
...
exports.forgetPassword = async (req, res, next) => {
  ...
  try {
    ...
    // Enregistrer le code de réinitialisation dans la base de données pour l'utilisateur
    user.resetCode = resetCode;
    console.log("Avant sauvegarde : " + user.resetCode);
    await user.save();
    console.log("Après sauvegarde : " + user.resetCode);

    // Envoyer le code de réinitialisation par email
    await sendResetCodeByEmail(email, resetCode);

    res.status(200).json({ message: "Reset code sent successfully", resetCode });
  } catch (error) {
    console.error(error);
