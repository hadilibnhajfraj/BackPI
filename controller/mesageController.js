...
async function add(req, res, next) {
    upload.fields([{ name: 'image' }, { name: 'fichier' }])(req, res, async function (err) {
        if (err) {
        ...
        try {
            ...
            // Detect models for sender and receiver
            ...
            if (!envoieModel || !receiveModel) {
             ...
             const blockExistsSenderToReceiver = await Block.exists({
            ...
            const blockExistsReceiverToSender = await Block.exists({
            ...
            if (blockExistsSenderToReceiver || blockExistsReceiverToSender) {
                return res.status(403).send("Vous ne pouvez pas envoyer de message à cet utilisateur car il vous a bloqué.");
            }
            ...
            // Si une image a été téléchargée, ajoutez son chemin au message
            if (req.files && req.files['image']) {
                newMessage.image = req.files['image'][0].path;
            }

            // Si un fichier a été téléchargé, ajoutez son chemin au message
...
async function getUserConversations(req, res) {
    ...
    try {
        ...
        // Iterate through the messages and extract the IDs of senders and receivers
        for (const message of messages) {
            if (message.id_user_envoie && message.id_user_envoie.toString() !== userId) {
                userIds.add(`${message.id_user_envoie}-${message.envoieModel}`);
            }
            if (message.id_user_receive && message.id_user_receive.toString() !== userId) {
        ...
        const users = await Promise.all(userIdArray.map(async (userId) => {
        ...
        const validUsers = users.filter(user => user).map(user => ({
            _id: user._id,
...
async function afficherConversation(req, res) {
    const id_user1 = req.params.id1;
    const id_user2 = req.params.id2;
    ...
    try {
      ...
      }).sort({ timestamp: 1 }); // Trier par timestamp dans l'ordre croissant
  
      if (messages.length === 0) {
        return res.status(404).json({ message: 'Aucun message trouvé entre ces utilisateurs.' });
      } else {
         // Mettre à jour les messages non lus à l'état "lu"
         for (let message of messages) {
            if (message.etat === 'non lu') {
                message.etat = 'lu';
                await message.save(); // Sauvegarder le message mis à jour dans la base de données
            }
        }
  ...
  async function obtenirUtilisateursEtEtudiantsActives(req, res) {
    try {
      ...
      console.error('Erreur lors de la récupération des utilisateurs et étudiants activés:', error);
      return res.status(500).json({ error: 'Erreur lors de la récupération des utilisateurs et étudiants activés' });
    }
  }
  ...
  async function bloquerUtilisateur(req, res) {
    ...
    if (!blockerId || !blockedId) {
      return res.status(400).json({ error: 'Les IDs blockerId et blockedId sont requis.' });
    }
    ...
    try {
      ...
      let blockerModel = blockerUser ? 'User' : blockerEtudiant ? 'Etudiant' : null;
      let blockedModel = blockedUser ? 'User' : blockedEtudiant ? 'Etudiant' : null;
      ...
      const existingBlock = await Block.findOne({
        $or: [
          ...
          { blocker: blockedId, blockerModel: blockedModel, blocked: blockerId, blockedModel: blockerModel }
        ]
      });
    ...
    try {
        ...
        if (unreadMessages.length > 0) {
            const notifications = unreadMessages.map(message => {
                return {
                    sender: `${message.id_user_envoie.firstName} ${message.id_user_envoie.lastName}`,
                    content: message.message
                };
            });
            res.status(200).json({ notifications, notification: `Vous avez ${unreadMessages.length} nouveaux messages.` }); // Envoyer les notifications et la notification dans la réponse
        } else {
            res.status(200).json({ notifications: [], notification: "Aucune nouvelle notification." }); // Si aucune notification n'est trouvée
        }
    } catch (error) {
        console.error("Erreur lors de la génération de la notification:", error);
        res.status(500).json({ error: 'Erreur lors de la génération de la notification.' }); // En cas d'erreur
    }
}
